...
SWFUpload.prototype.flashReady=function(){
    var a=this.getMovieElement();if(!a){
				...
				switch(errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							...
							this.queueData.errorMsg += '\n上传数量超过(' + settings.queueSizeLimit + ').';
						}
						break;
...
//                                alert('#'+file.id);
                                setTimeout(document.getElementById(file.id).style.display ="none",1000);
			}
